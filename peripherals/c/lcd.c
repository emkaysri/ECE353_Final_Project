...
********************************************************************************
* Summary:  This function will be used to print one (1) line of image data
* Returns:
*  Nothing
*******************************************************************************/
void lcd_draw_color_image(uint16_t image_width, uint16_t row_number, uint8_t *image_data ) {
	uint16_t i;
	//250 row width
	lcd_set_pos((240 - image_width) / 2 , ((240 + image_width) / 2), row_number, row_number);

	for(i=0; i < image_width * 3; i = i+3){
		// Red 5 MS bits
		// Green 6 MS bits
		// Blue 5 MS bits
		// data = Red, Green, Blue
		lcd_write_data_u16(((image_data[i] >> 3) << 11) | ((image_data[i + 1] >> 2) << 5) | (image_data[i + 2] >> 3));
	}
}
...
********************************************************************************
* Summary:  Draws a filled rectangle centered at x_center, y_center
* Returns:
*  Nothing
*******************************************************************************/
void lcd_draw_rectangle_centered (uint16_t x_center, uint16_t x_len, uint16_t y_center, uint16_t y_len, uint16_t fg_color){
  uint16_t i, j;
  lcd_set_pos(x_center - x_len/2, x_center + x_len/2, y_center - y_len/2, y_center + y_len/2);
  for(i = 0; i < x_len; i++){
		for(j = 0; j < y_len; j++){
